...
function openMenuAnimation_1() {
    if (!menuDisappearComplete_1) { 
        ...
        // If the menu is already open, skip the animation
        return;
    }

    // Delay toggling the class after the animation completes
    setTimeout(() => {
        if (!menuAppearComplete_1) {
            $('#takeover-nav').addClass("shown");
        }
        state_1 = "arrow";
        arrowDisappearComplete_1 = false;
        menuAppearComplete_1 = false;
    }, (menuAppearDurationInFrames_1 + arrowAppearDurationInFrames_1) * segmentDuration_1);
}
...
function closeMenuAnimation_1() {
    ...
    // Delay closing the menu after the animation completes
    setTimeout(() => {
        $('#takeover-nav').removeClass("shown");
        state_1 = "menu";
        menuDisappearComplete_1 = false;
        arrowAppearComplete_1 = false;
    }, menuAppearDurationInFrames_1 * segmentDuration_1);
}
...
// Add an event listener to elements with class .text-decoration-none
document.querySelectorAll('.text-decoration-none').forEach(function(item) {
    item.addEventListener('click', function(event) {
        const href = item.getAttribute('href');
        const scrollOffset = parseInt(item.getAttribute('data-scroll-offset')) || 0; // Default to 0 if attribute is not present
        ...
        if (href.startsWith('#')) {
            ...
            if (document.getElementById(sectionId)) {
                ...
                // Close the menu immediately after starting the scroll animation
                closeMenuAnimation_1();
